...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        count_run_calories = ((self.CALORIES_MEAN_SPEED_MULTIPLIER_RUNNING * self.get_mean_speed()
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получаем кол-во израсходованных каллорий за тренировку."""
        count_walk_calories = ((self.CALORIES_MEAN_SPEED_SHIFT_WALKING * self.weight 
                                 + ((self.get_mean_speed() * self.KMH_IN_MS)**2 
                                 / (self.height / self.CM_IN_M)
                                 * self.CALORIES_MEAN_SPEED_SHIFT_WALKING * self.weight)) 
                                 * (self.duration * self.MIN_IN_HOUR))
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получаем кол-во израсходованных каллорий за тренировку."""
        count_swim_calories = ((self.get_mean_speed() + self.CALORIES_MEAN_SPEED_MULTIPLIER_SWIMMING) 
                                     * self.CALORIES_MEAN_SPEED_SHIFT_SWIMMING * self.weight 
                                     * self.duration)
